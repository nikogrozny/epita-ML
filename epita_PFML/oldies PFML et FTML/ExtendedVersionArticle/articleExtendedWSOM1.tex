...
This work follow a double purpose: on one hand we study a specific, well-defined corpus, which is based on a set of scientific texts from the XV\up{th} century, on the other hand, we introduce a new tool, which can be used for any problem of lexicometry - or any problem involving contigency matrices.
...
\node[draw] (BNF25) at (15.525,5.5){{BNF25}};
\node[draw] (BNF1259) at (25.825,30.975){{BNF1259}};
...
\end{tikzpicture}
\caption{Projection on first two factors of the FCA. The eight texts appear in frames, a few words are displayed while the remaining are simply figured by dots, for the sake of readability.}
...
\begin{scope}[every node/.style={font={\scriptsize}}]
\node at (32,\ydlcorner+1){\textbf{ Factor 3 (?\%)}};
\node at (\xorg-\stepunit,\ydlcorner+1){\textbf{-1}};
\node at (\xorg,\ydlcorner+1){\textbf{0}};
\node at (\xorg+\stepunit,\ydlcorner+1){\textbf{1}};
\node[rotate=90] at (\xdlcorner+1.7,7){\textbf{ Factor 4 (?\%)}};
...
\node[draw] (BNF25)at (25.675,14.825){{BNF25}};
\node[draw] (BNF1259)at (34.825,14.725){{BNF1259}};
...
%\node[draw] (BNF25)at (24.445,26.17){BNF25};
%\node[draw] (BNF1259)at (35.53,21.55){BNF1259};
...
We display below (table~\ref{tab:koho1}) the SOM map which simultaneously represents the words and the texts. %One can observe that the interpretation is very silmilar to the interpretation that could be done from the Factorial Correspondence Analysis projections.
...
\hline
minutes super* & & notes & calculer cubic & fois mettre \cellcolor{grisclair} \textbf{BNF1259} & & {contraire} {depenser} {falloir} {meme} {racine} & aller donc ensuivre {savoir} & {multiplier} & {regle} venir\\
\hline
gecter & & & & & dessous & barrater {demi} & somme & voir & assembler\\
\hline
 & \cellcolor{grisclair} \textbf{BSG3143} & parteur & defaillir duplation mediation nommer numeration senestre & semblables & & circulaires demeurer derenier disaine ecrire entendre formes nombrateur oter prouver & entrer laisser rien & emprunter figure regarder & figure de~non~rien fraction muer rayes retenir\\
\hline
...
\hline
bref gectons {multipli-}{cation} {pratique} {seulement} & generale latin {nombrer} proportion reduction unite & soustraction & denomi- nateur & entier & ajoutement & & remotion \cellcolor{grisclair} \textbf{Kadran} & & \cellcolor{grisclair} \textbf{BNF25}\\
\hline
chose& nulle & {ensemble} {partie} & \cellcolor{grisclair} \textbf{Nantes456} & & & & {partement} {valoir} & & etre\\
\hline
arithmetique {compter} preuve tenir & cubbement destre digit diviser diviseur division lignes nombre compo{se}querir & \textbf{\textcolor{red}{DIRE}} {figurer} poser {science} & & & abreger lever precedent quotiens & numerateur & moindre nombre \cellcolor{grisclair} \textbf{Nicolas Chuquet} & partir plus \textbf{\textcolor{red}{RAISON}} & \\
\hline
...
\hline
algorisme article carrees cercle envient ligne pair sain \cellcolor{grisclair} \textbf{BNF1339} & addition chiffre former & {double} {doubler} moitie & appeler {donner} {maniere} {pouvoir} {se}& bailler demander mises nomper pareillement vouloir & & egale faire montrer necessaire romp selon & & & \\
\hline
cautelle & demontrer dessus & {garder} {regle} {de} {trois} & aliquot composer corps moins sub* toutefois & partant plaisir proportionel- lement requerir residu & appartenir convenir demande difference egaulx maieur millions rate survendre tant & & naturel roupt \cellcolor{grisclair} \textbf{Traicte praticque} & & fausse\\
\hline
\end{tabular}
}
\caption{Example of Kohonen Map. Texts are in bold and their cells are colored. Notice that \textit{raison} and \textit{dire} are far apart from each other.}
\label{tab:koho1}
\end{table}
...
\hline
 cautelle latin nombrer pair & demontrer & dessus \cellcolor{grisclair} \textbf{BNF1339} & addition traiter & compter double doubler somme & & devoir droit multiplier rien & barrater contraire depenser falloir meme racine & & calculer cubic fois mettre \cellcolor{grisclair} \textbf{BNF1259} \\ \hline
 ...
 ajoutement apparaitre diminution ensemble partie precedent proposer semblables & science soustraction & \textbf{\textcolor{red}{DIRE}} figurer poser & ajouter moitie position pouvoir \textbf{\textcolor{red}{RAISON}} se soustraire & moins regle de trois & partement & fausse & exemple & avaluer derenier difficile entrer repondre Kadran & \\ \hline
\cellcolor{grisclair} \textbf{Nantes456} & & & assembler grand valoir & appeler composer donner & aller bailler mises & convenir demande demander donc nomper pareillement & partant & & \\ \hline
...
SOM offers more synthetic perception of the information contained in the lexical table (\ref{tab:koho1}), but to clarify our purpose we've done a simplification of SOM. %(fichier : kohonen \_robust\_schema.svg ) 
...
\begin{tabular}{|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|p{1.9cm}|}
\hline
minutes super* & & notes & calculer cubic & fois mettre \cellcolor{grisclair} \textbf{BNF1259} & &  \textcolor{gris}{contraire}  \textcolor{gris}{depenser}  \textcolor{gris}{falloir}  \textcolor{gris}{meme}  \textcolor{gris}{racine} & aller donc ensuivre  \textcolor{gris}{savoir} &  \textcolor{gris}{multiplier} &  \textcolor{gris}{regle} venir\\
\hline
...
\hline
bref gectons  \textcolor{gris}{multipli-} \textcolor{gris}{cation}  \textcolor{gris}{pratique}  \textcolor{gris}{seulement} & generale latin  \textcolor{gris}{nombrer} proportion reduction unite & soustraction & denomi- nateur & entier & ajoutement & & remotion \cellcolor{grisclair} \textbf{Kadran} & & \cellcolor{grisclair} \textbf{BNF25}\\
\hline
chose& nulle &  \textcolor{gris}{ensemble}  \textcolor{gris}{partie} & \cellcolor{grisclair} \textbf{Nantes456} & & & &  \textcolor{gris}{partement}  \textcolor{gris}{valoir} & & etre\\
\hline
arithmetique  \textcolor{gris}{compter} preuve tenir & cubbement destre digit diviser diviseur division lignes nombre compo \textcolor{gris}{se}querir &  \textcolor{gris}{dire}  \textcolor{gris}{figurer} poser  \textcolor{gris}{science} & & & abreger lever precedent quotiens & numerateur & moindre nombre \cellcolor{grisclair} \textbf{Nicolas Chuquet} & partir plus  \textcolor{gris}{raison} & \\
\hline
...
It's the same for two other words ("raison", "dire" \textit{to say}). % commenter raison et dire. 	
...
It is the articulation between these two levels which makes this representation interesting. In the end, the meaning of this new kind of factorial map is quite intuitive and offers easy tools to the argumentation.
...
\end{tikzpicture}
\caption{Correlation between fickleness and distance to the center. x-axis represents the number of fickle pairs a word belongs to, while y-axis stands for the square distance to the center of the analysis $d^2=\sum_{i\leq 7} f_i^2$}
\end{figure}
...
%&compter&0&.1{\tiny 75}&.0{\tiny 75}&0&.0{\tiny 5}&0&.1&.1{\tiny 25}&.0{\tiny 25}&0&.0{\tiny 5}&.0{\tiny 25}&.1{\tiny 75}&\cellcolor{gray}.3{\tiny 75}&.1{\tiny 5}&\cellcolor{gray}.6{\tiny 5}&\cellcolor{gray}.6{\tiny 25}&\cellcol